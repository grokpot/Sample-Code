...
"""
This was a project for Software Engineering. The specs are here: http://www.spoj.pl/problems/PROBTNPO/
"""
# ---------------------------
...
# ------------
# collatz_read  -   Written by Professor Downing
# ------------
...
# ------------
# compute_cycle_length  -   Written by me
# ------------

# Builds a cache which is an array with indicies of n and values of cycle length(n), this allows for constant time lookup.
...
def compute_cycle_length (current_num):
    ...
    current_num is an int in the range from input for which the cycle length is computed
    ...
    # If the current num is within the cache list index and has been cached, add the cached value
    ...
    # Otherwise recurse
        ...
        # Else current_num is odd
            ...
            # NOTE: This is a shortcut. Since an odd * odd = even, we can combine steps here (as opposed to sending 3n+1 and then n/2 in the next step). Thus TWO_CYCLES.
...
def collatz_eval (i, j) :
    ...
    # Instantiation checked as a post-condition
    max_cycle_length = -1

    # Sets the parameters in the right order for Python range() (also makes the range inclusive)
    if i < j+1:
        n_range = range(i, j+1)
    else:
        n_range = range(j, i+1)
...
# -------------
# collatz_print  -   Written by Professor Downing
# -------------
...
# -------------
# collatz_solve  -   Written by Professor Downing
# -------------
